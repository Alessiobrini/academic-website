...
- `_pages/` — top-level pages: `about.md` (homepage), `publications.md`, `cv.md`, `teaching.md`, `projects.md`, `repositories.md`. Each declares `permalink:` and nav order in front matter.
...
- `_data/` — structured YAML: `cv.yml` (CV content), `coauthors.yml`, `venues.yml`, `repositories.yml`.
...
- **Updating the CV**: edit `_data/cv.yml` for the structured CV page; `_pages/cv.md` is the page wrapper.
